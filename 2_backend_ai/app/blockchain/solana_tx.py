...
async def send_audit_record(model_id: str, trust_score: int, decision: str):
    ...
    root_dir = current_dir.parent.parent.parent   # Поднимаемся на 3 уровня вверх до корня

    # Собираем точный путь к IDL
    idl_path = root_dir / "1_solana_program" / "target" / "idl" / "annex_iv_registry.json"
...
if __name__ == "__main__":
    ...
    asyncio.run(send_audit_record(
        model_id="GPT-4-Finance-Bot", 
        trust_score=85, 
